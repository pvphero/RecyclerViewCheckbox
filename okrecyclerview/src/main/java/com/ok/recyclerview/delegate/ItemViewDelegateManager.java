...
public class ItemViewDelegateManager<T> {
    ...
    public int getItemViewType(T item,int position){
        ...
        throw new IllegalArgumentException(
                "No ItemViewDelegate added that matches poistion="+position+" in data source"
        );
    }
    ...
    public void onBind(ViewHolder holder,T item, int position, List<Object> payloads){
        ...
        for (int i=0;i<delegatesCount;i++){
            ItemViewDelegate<T> delegate=delegates.valueAt(i);
            if (delegate.isForViewType(item,position)){
                delegate.onBind(holder,item,position,payloads!=null?payloads:PAYLOADS_EMPTY_LIST);
                return;
            }
        }
        throw new IllegalArgumentException(
    ...
    public int getItemViewLayputId(int viewType){
